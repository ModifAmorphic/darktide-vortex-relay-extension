...
/**
 * Relay launcher executable filename, and the single Relay contract this
 * script enforces. Inlined rather than imported from `src/constants`:
 * Node type-stripping does not resolve extensionless `.ts` imports, and
 * adding `.ts` suffixes would require a tsconfig change.
 */
...
/**
 * Subset of the GitHub release asset shape this script consumes, defined
 * locally to keep the pure helpers decoupled from the full API response.
 */
interface Asset {
  id: number;
  name: string;
 ...
 * Exported for unit testing. Throws a {@link UserError} naming the tag
 * and the available asset names when no asset matches, and a separate
 * error when more than one matches (a release bug that should not be
 * silently resolved).
 */
...
/** Fetches the releases list; validation happens in {@link selectLatestRelease}. */
...
/** Returns the Authorization header when `GITHUB_TOKEN` is set, else `{}` so the spread is a no-op. */
function authHeaders(): Record<string, string> {
  const token = process.env.GITHUB_TOKEN;
  return token && token.length > 0 ? { Authorization: `Bearer ${token}` } : {};
}

/** Streams the download to `destPath` so the full zip is never buffered in memory. */
...
/** Extracts `zip` into `dest` via `Expand-Archive -LiteralPath` (a specific file, no wildcards). */
...
/** Runs a PowerShell command via `powershell.exe`. */
 ...
 * quotes per PowerShell escaping rules, for safe `-LiteralPath` /
 * `-DestinationPath` arguments.
 */
...
async function main(): Promise<void> {
  try {
    ...
    try {
      ...
      // Replace the target directory so re-runs land the new release verbatim.
